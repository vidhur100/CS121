...
def letterCounts(fname):
    ...
    count_pairs = [(int(count), char) for char, count in cts.items() if isinstance(count, int) or (isinstance(count, str) and count.isdigit())]
    count_pairs.sort(reverse=True, key=lambda x: x[0])

    for count, char in count_pairs:
        print(f'({count}, \'{char}\')')
