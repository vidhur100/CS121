...
def fourBitAdd(xxxx,yyyy):
    ...
    and c is the carry.'''
    (c1, xo) = FA(xxxx[3], yyyy[3], 0)
    (c2, xt) = FA(xxxx[2], yyyy[2], c1)
    (c3, xf) = FA(xxxx[1], yyyy[1], c2) 
    (c4, xe) = FA(xxxx[0], yyyy[0], c3)
    return (c4, (xe, xf, xt, xo))

# EXERCISE: implement the following.
def test_fourBitAdd():
    '''tests the four bit add function'''
